...
function Get-AsterAuthenticodeObservation {
    param(
    ...
    $safePath = Resolve-AsterContainedPath -RepositoryRoot $RepositoryRoot -Path $Path
    $availableModules = @(Get-Module -ListAvailable -Name "Microsoft.PowerShell.Security")
    $securityManifest = Join-Path `
        $PSHOME `
        "Modules\Microsoft.PowerShell.Security\Microsoft.PowerShell.Security.psd1"
    if ($availableModules.Count -eq 0 -or -not (Test-Path -LiteralPath $securityManifest -PathType Leaf)) {
        return "NOT RUN (verifier unavailable)"
    }
    try {
        $securityModules = @(Import-Module $securityManifest -Force -PassThru -ErrorAction Stop)
    }
    catch {
        return "NOT RUN (verifier unavailable)"
    }
    if ($securityModules.Count -ne 1) {
        throw "The Authenticode verifier contract is invalid."
    }
    $securityModule = $securityModules[0]
    $command = $securityModule.ExportedCommands["Get-AuthenticodeSignature"]
    if (
        $null -eq $command -or
        $command.CommandType -ne [System.Management.Automation.CommandTypes]::Cmdlet -or
        $command.ModuleName -cne "Microsoft.PowerShell.Security"
    ) {
        throw "The Authenticode verifier contract is invalid."
    }
    $signatures = @(& $command -LiteralPath $safePath -ErrorAction Stop)
    if (
        $signatures.Count -ne 1 -or
        $signatures[0].Status -isnot [System.Management.Automation.SignatureStatus]
    ) {
        throw "The Authenticode verifier returned an invalid result."
    }
    $status = $signatures[0].Status.ToString()
    $allowedStatuses = [System.Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
    foreach ($allowed in @(
            "Valid",
            "UnknownError",
            "NotSigned",
        ...
        [void]$allowedStatuses.Add($allowed)
    }
    if (-not $allowedStatuses.Contains($status)) {
        throw "The Authenticode verifier returned an invalid status."
    }
    [void](Resolve-AsterContainedPath -RepositoryRoot $RepositoryRoot -Path $safePath)
    return $status
}
    ...
    try {
        ...
        $identityRows = foreach ($path in $identityTargets) {
            ...
            $signature = if ($item.Extension -eq ".exe") {
                Get-AsterAuthenticodeObservation -RepositoryRoot $root -Path $safePath
            }
            else {
                "Not applicable"
            }
...
Authenticode is a host-tool observation only. ``NOT RUN (verifier unavailable)``
means the PowerShell security module could not be loaded; it is not signature
evidence and never satisfies the production signing gate.
...
| File | Bytes | Modified UTC | SHA-256 | Authenticode observation |
| --- | ---: | --- | --- | --- |
